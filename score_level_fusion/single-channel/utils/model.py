...
def train_model(dataloaders, dataset_sizes, class_names, device,
    ...
    for weight in weight_enum:
        ...
        # logging.info(model.conv1.weight.data)
        ...
        # Change the output layer of the model to the number of classes
...
def evaluate_model(model_path, dataloaders, device):
    ...
    with torch.no_grad():
        for inputs, labels in dataloaders['val']:
            ...
            pred_weights.extend(outputs.cpu().numpy())
    ...
    return accuracy, predictions, real_labels, pred_weights
